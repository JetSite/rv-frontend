...
export const Contacts: FC<Props> = ({ data, locale }) => {
  ...
  return (
    <div className="max-w-content desktopOnly:max-w-[988px] mx-auto flex desktop:flex-row flex-col gap-8 mobile:px-7 tablet:px-8 tablet:w-2/3 ">
        ...
        <ul className="flex flex-col gap-4 desktopOnly:gap-3 desktopOnly:mb-10 mb-14">
          {pageData.importantContacts.map(contact => (
            <li key={contact.id}>
              <Link
                href={contact.link}
                className="flex gap-4 desktopOnly:gap-3 items-center relative w-fit before:absolute before:h-0.5 before:content-[ ] before:bg-h before:left-0 before:-bottom-2 hover:before:w-full before:block"
              >
